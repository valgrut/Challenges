...
int main(int argc, char *argv[]) 
{
    ...
    vector<uint_fast8_t>* lanfish_timers = new vector<uint_fast8_t>;
    ...
    for(int i = 0; i < days_of_simulation; i++)
    {
        ...
        {
            bool newf = false;
            if(lanfish_timers->at(fish_i) == 0)
            {
                lanfish_timers->at(fish_i) = 6;
            ...
            if(newf == false)
            {
                lanfish_timers->at(fish_i) -= 1;
            }
        }
        
        cout << "After " << i+1 << " days: " << lanfish_timers->size() << endl;
    }
    delete lanfish_timers;
    fdata.close(); 
}
